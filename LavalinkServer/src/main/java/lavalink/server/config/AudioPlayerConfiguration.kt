...
import lavalink.server.extsource.spotilava.SpotiLavaSourceManager
...
class AudioPlayerConfiguration {
    ...
    fun audioPlayerManagerSupplier(sources: AudioSourcesConfig, serverConfig: ServerConfig, routePlanner: AbstractRoutePlanner?): AudioPlayerManager {
        ...
        if (sources.isLavaSpotify) {
            log.info("Enabled lavaSpotify extensions with spotiLavaUrl " + serverConfig.spotiLavaUrl)
            audioPlayerManager.registerSourceManager(SpotiLavaSourceManager(serverConfig.spotiLavaUrl))
        }
